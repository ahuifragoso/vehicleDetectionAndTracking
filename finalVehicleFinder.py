...
spatial = 32
histbin = 32
colorspace = 'HLS'# Can be RGB, HSV, LUV, HLS, YUV
#*****HOG********
orient = 9
pix_per_cell = 8
cell_per_block = 8
...
def draw_labeled_bboxes(img, labels):
    ...
    for car_number in range(1, labels[1]+1):
        ...
        cv2.circle(img, (centroidX,centroidY), 10, (1,0,1), thickness=3)        
    # Return the image
    return img, centroidX, centroidY
...
#step9: Enjoy
lista = glob.glob('testImages/test*.jpg')
pos = 0
bandera = 0
for idx, fname in enumerate(lista):
    #reduce my fishEyeError
    shashin = mpimg.imread(fname)
    ...
    mado,ventanas = slideWindows(shashin, xStartStop=[650, 1280], 
                                yStartStop=[400, 
                                700], 
                                xy_window=(100,100), 
                                xy_overlap=(0.5,0.5))
    ...
    draw_img = draw_boxes(shashin, ventanaFinal)
    ...
    if pos > 0:
        centX, centY = findCentroid(labels)
        centXpre, centYpre = findCentroid(labelsPre)
        print(centX,centY)
        print("--------")
        print(centXpre,centYpre)
        print("--------")
        diferenciaX = centX - centXpre
        diferenciaY = centY - centYpre
        print("diferencia: ", (diferenciaX + diferenciaY)/2)        
        print("******************")
    else:
        labelsPre = labels
    
    draw_img, centX, centY = draw_labeled_bboxes(draw_img, labels)
